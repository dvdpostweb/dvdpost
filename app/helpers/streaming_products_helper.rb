module StreamingProductsHelper
  def flowplayer(source_file, source, streaming, token_name, browser)
      if browser.iphone? || browser.ipad?
        ...
        script = <<-script
        $("#player").html("<object width='696' height='389'><param name='movie' value='http://vod.dvdpost.be/StrobeMediaPlayback.swf'></param><param name='flashvars' value='src=http://vod.dvdpost.be/#{token_name}_#{streaming.languages.by_language(:fr).first.short_alpha}_#{streaming.subtitles.count > 0 ? streaming.subtitles.by_language(:fr).first.short_alpha : 'non'}.f4m&loop=true&autoPlay=true&streamType=recorded&verbose=true&initialBufferTime=5&expandedBufferTime=30'></param><param name='allowFullScreen' value='true'></param><param name='allowscriptaccess' value='always'></param><embed src='http://vod.dvdpost.be/StrobeMediaPlayback.swf' type='application/x-shockwave-flash' allowscriptaccess='always' allowfullscreen='true' width='696' height='389' flashvars='src=http://vod.dvdpost.be/#{token_name}_#{streaming.languages.by_language(:fr).first.short_alpha}_#{streaming.subtitles.count > 0 ? streaming.subtitles.by_language(:fr).first.short_alpha : 'non'}.f4m&loop=true&autoPlay=true&streamType=recorded&verbose=true&initialBufferTime=5&expandedBufferTime=30'></embed></object>")
        script
      end
