class VodWishlistsController < ApplicationController
  def index
    @token_list = current_customer.vod_wishlists.find(:all, :joins => [{:products => :descriptions}, :streaming_products, :tokens], :group => 'vod_wishlists.imdb_id', :order =>'products_description.products_name', :conditions => ["streaming_products.available_from < :time and streaming_products.expire_at > :time and streaming_products.status = 'online_test_ok' and products_status != -1 and products_type = :type and tokens.updated_at > :start and tokens.updated_at < :end and tokens.customer_id = :customer_id", {:time => Time.now, :type => DVDPost.product_kinds[params[:kind]], :start => 2.days.ago.localtime, :end => Time.now, :customer_id => current_customer.to_param}])
