class VodWishlistsController < ApplicationController
  def index
    ...
    @soon_list = current_customer.vod_wishlists.find(:all, :joins => [{:products => :descriptions}], :group => 'vod_wishlists.imdb_id', :order =>'products_description.products_name', :conditions => ["products_media = :media and vod_next = 1 and products_status != -1 and products_type = :type", {:time => Time.now, :type => DVDPost.product_kinds[params[:kind]], :start => 2.week.ago.localtime, :end => Time.now, :media => DVDPost.product_types[:vod]}])
