class HomeController < ApplicationController
  ...
  def get_data(kind)
    ...
    landing_serial = when_fragment_expired "#{Rails.env}_landings_hp4_#{I18n.locale}_#{params[:promo]}_#{params[:kind]}_#{ENV['HOST_OK']}", 30.minutes.from_now.localtime do
    ...
    if(kind == :adult)
      ...
      Rails.logger.debug { "@@@#{trailer_limit}" }
      Rails.logger.debug { "@@@#{@trailers_week.inspect}" }
      Rails.logger.debug { "@@@#{@trailers_week.count}" }
