class RatingsController < ApplicationController
  def create
    ...
    Customer.send_evidence('ItemRated', params[:product_id], current_customer, request,{:responseid => params[:response_id], :segment1 => params[:source], :formFactor => format_text(@browser) , :rule => params[:source]}, {:rating => params[:value]})
